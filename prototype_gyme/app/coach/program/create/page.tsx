...
async function createProgram(payload: CreateProgramPayload, token: string) {
  ...
  return (await res.json()) as CreateProgramResponse;
}
    ...
    <div className="grid gap-4 lg:grid-cols-[220px_1fr] lg:items-start">
      ...
      <div className="grid gap-4">
        ...
        <p className="text-sm leading-6 text-white/75">
          Upload a clean cover image for the program. The file is stored on the server and the returned URL is saved with the program.
        </p>
...
export default function CreateProgramPage() {
  ...
  const payload = useMemo<CreateProgramPayload>(() => {
    return {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await createProgram(payload, token);
      const id = result?.id;
        ...
        <div className="mx-auto max-w-6xl px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
          <div className="relative max-w-4xl">
            ...
            <p className="mt-5 max-w-3xl text-base leading-7 text-white/75 sm:text-lg">
              Build a new program with a clean workflow, structured fields, and a cover image that is uploaded and saved automatically.
            </p>

            <div className="mt-8 flex flex-wrap gap-3">
              {["Coach-owned", "Track-based", "Image upload", "Professional UI"].map(
                (item) => (
                  <span
                    key={item}
