...
type ScopeFilter = "all" | "global" | "custom";

const API_URL = process.env.NEXT_PUBLIC_API_URL || "https://fitzone-16.runasp.net";

const ENDPOINTS = {
  list: `${API_URL}/api/Exercise`,
  create: `${API_URL}/api/Exercise`,
  update: (id: number) => `${API_URL}/api/Exercise/${id}`,
  remove: (id: number) => `${API_URL}/api/Exercise/${id}`,
};
...
export default function CoachExercisesPage() {
  ...
  async function loadExercises() {
    try {
      setLoading(true);
      setError("");
      const data = await apiRequest<PagedResponse<ExerciseListItem> | ExerciseListItem[]>(ENDPOINTS.list);
      setItems(normalizeExercisesResponse(data));
    } catch (err: any) {
  ...
  const stats = useMemo(() => {
    ...
    const avgLevel = total
      ? Math.round(
          items.reduce((sum, item) => sum + Number(item.fitnessLevel || 0), 0) / total
        )
      : 0;
    return { total, globals, custom, avgLevel };
  }, [items]);
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((ex) => {
      ...
      const scopeText = ex.isGlobal ? "global" : "custom";
  ...
  async function openEdit(exercise: ExerciseListItem) {
    if (exercise.isGlobal) {
      setError("لا يمكن تعديل تمرين عام");
      return;
    }

    try {
      setError("");
      const details = await apiRequest<any>(`${API_URL}/api/Exercise/${exercise.id}`);
      ...
      setForm({
        ...
        equipmentNeeded: normalizeEquipmentNeeded(details?.equipmentNeeded ?? exercise.equipmentNeeded),
        fitnessLevel: fitnessLevelToValue(details?.fitnessLevel ?? exercise.fitnessLevel),
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (mode === "edit" && selected?.isGlobal) {
      setError("لا يمكن تعديل تمرين عام");
      return;
    }
  ...
  async function handleDelete(exercise: ExerciseListItem) {
    if (exercise.isGlobal) {
      setError("لا يمكن حذف تمرين عام");
      return;
    }

    const ok = window.confirm(`متأكد إنك عايز تمسح التمرين: ${exercise.name} ؟`);
    if (!ok) return;
    ...
    try {
      ...
      await apiRequest<void>(ENDPOINTS.remove(exercise.id), {
        method: "DELETE",
      });
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-24 sm:px-6 lg:px-8 lg:py-28">
          <Reveal>
            <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
              <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-xs font-semibold uppercase tracking-[0.28em] text-white/80">
                <Sparkles className="h-4 w-4 text-[#84FF00]" /> Elite Fitness Platform
              </div>

              <button
                onClick={openCreate}
                className="group relative inline-flex items-center gap-2 overflow-hidden rounded-full bg-[#84FF00] px-5 py-2.5 text-xs font-bold uppercase tracking-[0.16em] text-black shadow-[0_8px_30px_rgba(132,255,0,0.35)] transition hover:shadow-[0_8px_40px_rgba(132,255,0,0.55)] active:scale-[0.98]"
              >
          ...
          <Reveal delay={80}>
            <div className="max-w-4xl">
              <h1 className="text-4xl font-black uppercase tracking-tight sm:text-5xl lg:text-7xl">
                EXERCISES <span className="text-[#84FF00]">LIBRARY</span>
              </h1>
          ...
          <Reveal delay={140}>
            <div className="mt-8 flex flex-wrap gap-3">
              <StatPill icon={<Activity className="h-4 w-4" />} label="Exercises" value={stats.total} />
              <StatPill icon={<BadgeCheck className="h-4 w-4" />} label="Global" value={stats.globals} />
              <StatPill icon={<Layers3 className="h-4 w-4" />} label="Custom" value={stats.custom} />
              <StatPill icon={<Target className="h-4 w-4" />} label="Avg Level" value={stats.avgLevel} />
            </div>
          </Reveal>
        </div>
      </section>
        ...
        <Reveal>
          <div className={cardClass("p-4 sm:p-5") }>
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              <select value={levelFilter} onChange={(e) => setLevelFilter(e.target.value as LevelFilter)} className={inputClass}>
              ...
              <select value={scopeFilter} onChange={(e) => setScopeFilter(e.target.value as ScopeFilter)} className={inputClass}>
              ...
              <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white/75">
                <span>Use responsive cards on mobile</span>
          ...
          {loading ? (
            ...
            <Reveal delay={120}>
              <div className={cardClass("p-12 text-center") }>
                ...
                <button
                  onClick={openCreate}
                  className="mx-auto mt-6 inline-flex items-center gap-2 rounded-full bg-[#84FF00] px-5 py-2.5 text-xs font-bold uppercase tracking-[0.16em] text-black transition hover:shadow-[0_8px_40px_rgba(132,255,0,0.45)]"
                >
                  <Plus className="h-4 w-4" />
            ...
            <Td className="text-right">
              <div className="flex flex-wrap justify-end gap-2">
                <button
                  ...
                  title={
                    exercise.isGlobal
                      ? "لا يمكن تعديل تمرين عام"
                      : "Edit"
                  }
                  className="inline-flex h-10 items-center rounded-xl border border-[#84FF00]/35 bg-white/[0.03] px-4 text-sm font-semibold text-white transition hover:bg-[#84FF00] hover:text-black disabled:cursor-not-allowed disabled:opacity-50"
                >
                  ...
                  <article key={exercise.id} className={cardClass()}>
                    <div className="border-b border-white/10 p-5">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <p className="text-xs uppercase tracking-[0.22em] text-white/40">ID #{exercise.id}</p>
                          <h3 className="mt-2 text-2xl font-black uppercase tracking-tight">{exercise.name}</h3>
                        </div>
                        <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${statusClass(!!exercise.isGlobal)}`}>
                          {exercise.isGlobal ? "Global" : "Custom"}
                        </span>
                      </div>
                    ...
                    <div className="grid gap-3 p-5 sm:grid-cols-2">
                      ...
                      <InfoBox label="Level" value={normalizeFitnessLevelLabel(exercise.fitnessLevel)} tone="green" />
                    ...
                    <div className="border-t border-white/10 p-5">
                      <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        <button
                          onClick={() => handleDelete(exercise)}
                          disabled={deletingId === exercise.id || exercise.isGlobal}
                      ...
                      <div className="mt-3 grid gap-3 sm:grid-cols-2">
                        {exercise.videoUrl ? (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex h-11 items-center justify-center rounded-xl border border-[#84FF00]/35 bg-white/[0.03] px-4 text-sm font-semibold text-[#84FF00]"
                          >
                            <Video className="mr-2 h-4 w-4" /> Open Video
                          </a>
                        ) : (
                          <div className="inline-flex h-11 items-center justify-center rounded-xl border border-white/10 bg-white/[0.03] px-4 text-sm text-white/45">
                            No video link
                          </div>
                        )}

                        <div className="inline-flex h-11 items-center justify-center rounded-xl border border-white/10 bg-white/[0.03] px-4 text-sm text-white/70">
                          <Layers3 className="mr-2 h-4 w-4 text-[#00D9FF]" /> {exercise.isGlobal ? "Global" : "Custom"}
                        </div>
                      </div>
                    </div>
                  </article>
              ...
              {/* Floating create button for mobile, lives under the grid for thumb reach */}
              <button
                onClick={openCreate}
                className="fixed bottom-6 right-6 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-[#84FF00] text-black shadow-[0_10px_40px_rgba(132,255,0,0.45)] transition hover:scale-105 active:scale-95 lg:hidden"
      ...
      {open ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 px-4 py-6 backdrop-blur-sm">
          <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-[28px] border border-white/10 bg-[#0A0A0A] shadow-[0_20px_80px_rgba(0,0,0,0.6)]">
            <div className="sticky top-0 z-10 border-b border-white/10 bg-[#0A0A0A]/95 px-6 py-5 backdrop-blur-xl sm:px-7">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.24em] text-white/45">
                    <Dumbbell className="h-3.5 w-3.5 text-[#84FF00]" />
                    Exercise Form
                  </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-5 px-6 py-6 sm:px-7">
              {error ? (
                <div className="rounded-2xl border border-[rgba(255,107,0,0.28)] bg-[rgba(255,107,0,0.08)] p-3 text-sm text-[#FFB36B]">
                  {error}
                </div>
              ) : null}
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                <button
                  type="submit"
                  disabled={saving}
                  className="rounded-2xl bg-white px-5 py-3 font-semibold text-black transition hover:bg-[#84FF00] disabled:cursor-not-allowed disabled:opacity-70"
                >
                ...
                <button
                  type="button"
                  onClick={() => setOpen(false)}
                  className="rounded-2xl border border-[#84FF00] px-5 py-3 font-semibold text-[#84FF00] transition hover:bg-[#84FF00] hover:text-black"
                >
                  Cancel
                </button>
              </div>
...
function InfoBox({
  ...
  return (
    <div className="rounded-[20px] border border-white/10 bg-white/[0.03] p-4">
      ...
      <p className={`mt-2 text-sm font-medium ${tone === "green" ? "text-[#84FF00]" : tone === "cyan" ? "text-[#00D9FF]" : tone === "orange" ? "text-[#FFB36B]" : "text-white"}`}>
        {value ?? "-"}
      </p>
    </div>
  );
